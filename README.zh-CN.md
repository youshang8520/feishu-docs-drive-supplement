...
- 支持用户授权：
  - `auth status` 查看授权状态
  - `auth start` 生成授权链接
  - `auth send-link` 直接发送授权链接到飞书
  - `auth poll` 完成授权轮询
  - `auth import` 导入已有授权
- 当提供 `receive_id` 时，可以直接将授权链接发送到飞书聊天
- 提供以下功能的实用操作：
  - 云盘 (Drive)
  - 上传 (Upload)
  - 文档 (Docs)
...
```bash
python -m cc_feishu.cli auth send-link --receive-id <目标ID> --receive-id-type open_id
```

参数说明：
- `--receive-id`：目标用户或群组的 ID
- `--receive-id-type`：ID 类型，可选值：`open_id`、`chat_id`、`user_id`、`union_id`、`email`
...
可以。使用 `auth send-link` 命令可以直接将授权链接发送到指定的飞书用户或群组。
